...
public class ButtonSettings extends NodePreferenceActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Preference prefSlider = findPreference(
                    Constants.NOTIF_SLIDER_PANEL_KEY);

        Preference prefLegacySlider = findPreference(
                    Constants.NOTIF_SLIDER_LEGACY_PANEL_KEY);

        int feature = Constants.getNotificationSliderFeature();
        if (feature == Constants.NOTIF_SLIDER_FEATURE_FULL) {
            getPreferenceScreen().removePreference(prefLegacySlider);
            initNotificationSliderPreference();
        } else if (feature == Constants.NOTIF_SLIDER_FEATURE_LEGACY) {
            getPreferenceScreen().removePreference(prefSlider);
        } else {
            getPreferenceScreen().removePreference(prefSlider);
            getPreferenceScreen().removePreference(prefLegacySlider);
        }
